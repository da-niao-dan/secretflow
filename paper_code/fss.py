...
def G(random_bits: jnp.array, unpack=True) -> jnp.array:
    ...
    input shape (n,) uint32 jnp array
    output shape (n, 6) uint32 jnp array
    """

    # assume random_bits is a byte array of shape (n, )
    # convert byte array into keys
    keys = jax.vmap(jax.random.key)(random_bits)

    # split keys into 6
    # shape (n, 6) of keys
    six_keys = jax.vmap(lambda x: jax.random.split(x, 6))(keys)

    # generate 6 new random bit arrays of shape lambda | lambda | 1 | lambda | lambda | 1

    six_arrs = jax.vmap(lambda x: six_key_to_array(x, random_bits.dtype))(six_keys)
    if unpack:
